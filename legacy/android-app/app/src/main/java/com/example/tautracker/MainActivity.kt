        ...
        setContent {
            TauTrackerTheme {
                Surface(
                    ...
                    var database by remember { mutableStateOf<TauDatabase?>(null) }
                    ...
                    LaunchedEffect(Unit) {
                        withContext(Dispatchers.IO) {
                            ...
                            database = Room.databaseBuilder(
                                applicationContext,
                                TauDatabase::class.java, "tau-database"
                            ).build()
                        }
                    }

                    if (secureStorage == null || database == null) {
                        ...
                        val syncRepository = remember { SyncRepository(moodleApi, database!!) }
                        TauTrackerApp(secureStorage!!, syncRepository, moodleApi)
                    }
                }
            }
        }
    ...
    NavHost(navController = navController, startDestination = startDestination) {
        ...
        composable("dashboard") {
            val viewModel = remember {
                DashboardViewModel(syncRepository, secureStorage)
            }
            DashboardScreen(
                viewModel = viewModel,
        ...
        composable("settings") {
            com.example.tautracker.ui.settings.SettingsScreen(
                secureStorage = secureStorage,
