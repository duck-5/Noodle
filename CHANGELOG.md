...
## [Unreleased]
### Added
- **Automatic Mobile Web Service Token Renewal on Expiry (`packages/moodle-client`)**: Added `getMobileToken()` to `ScraperMoodleStrategy` and `setToken()` across all strategies. When `RestMoodleStrategy` encounters `invalidtoken` or `accessexception`, `fallbackRunner.ts` silently resets and acquires a fresh mobile web service token from `/user/managetoken.php` using active SSO session cookies, updates all strategies, resets circuit breaker cooldowns, and retries the REST request seamlessly without requiring the user to re-authenticate (Fixes #26, resolves D2).
- **Direct Course-Page Assignment Scraping (`packages/moodle-client`)**: Rewrote `ScraperMoodleStrategy.getAssignments()` to extract assignments directly from individual course pages (`/course/view.php?id=X`) across all discovered academic years. Decoupled assignment discovery from the Moodle Calendar (which filtered out past deadlines), guaranteeing all historical and current course assignments are captured (Fixes #26, resolves D4).

### Fixed
- **AJAX Course Year Mapping Bug (`packages/moodle-client`)**: Fixed an issue in `extractCoursesViaAjax` where courses added from timeline AJAX responses bypassed `this.courseYearMap`, preventing them from being scraped for assignments.
- **REST 0 Assignments Fallback Trigger (`packages/moodle-client`)**: Updated `RestMoodleStrategy.getAssignments()` to count total assignments across all courses and throw a fallback error if 0 assignments are returned, allowing scraper fallback to discover archive assignments.
